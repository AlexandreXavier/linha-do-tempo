...
const LoadingSpinner = () => (
    <div className="flex items-center justify-center h-full">
        <svg className="animate-spin h-8 w-8 text-[#D2691E]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const ErrorDisplay = () => (
    <div className="flex items-center justify-center h-full">
         <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-[#8B0000]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const Placeholder = () => (
    <div className="flex flex-col items-center justify-center h-full bg-white">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-2 text-[#5A4A3E]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, isMobile }) => {
    ...
    // Reset estados quando a URL da imagem muda ou o status muda para pending.
    useEffect(() => {
        if (status === 'pending') {
    ...
    // Quando a imagem é carregada, inicia a animação de desenvolvimento.
    useEffect(() => {
        if (isImageLoaded) {
    ...
    const handleDragStart = () => {
        // Reset velocity on new drag para prevenir gatilhos falsos de dados antigos
    ...
    const handleDrag = (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
        ...
        const shakeCooldown = 2000; // dois segundos cooldown para evitar spamming.

        const { x, y } = info.velocity;
        const { x: prevX, y: prevY } = lastVelocity.current;
        const now = Date.now();

        // A Verdade de um "shake" é uma movimentação rápida e uma mudança abrupta de direção.
        // Detectamos isso verificando se a velocidade é alta e se sua direção
        // foi invertida em relação ao último frame (ou seja, o produto escalar é negativo).
                    ...
                    <>
                        <div className={cn(
                            ...
                            {onDownload && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation(); //Previne o início do arrastar ao clicar
                                        onDownload(caption);
                                    }}
